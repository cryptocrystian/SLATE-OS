...
## Report-slot rendering (Sprint 1 · internal preview only)

`ExecutiveSummaryTwoByTwo` is now **prop-driven** — its proof-of-fit `SAMPLE_DATA` was lifted into `app/app/charts-preview/page.tsx` so the preview surface is visually unchanged, and the exhibit accepts the `ExecutiveSummaryPortfolioProps` shape produced by `lib/charts/adapters/executive-summary-2x2-adapter.ts`. All five Group-A exhibits are now report-slot eligible. The Sprint 1 renderer at [`components/reports/report-exhibit-slots.tsx`](../reports/report-exhibit-slots.tsx) mounts the adapter results inside the operator-only internal report preview at `/app/engagements/[id]/report`. Fallback cards render explicit operator-facing copy with deep links to the editing surface — never sample data — per `docs/17` § Fallback Rules. Group B is still not imported by the report-slot renderer; PDF / export and client-facing send / share remain behind `LockedActionButton`.

## Persisted report-section slot references (Sprint 2)

Render order is now driven by **persisted `report_sections.exhibit_slot` values**, not the static slot table. Migration [`supabase/migrations/0012_report_section_exhibit_slot.sql`](../../supabase/migrations/0012_report_section_exhibit_slot.sql) adds a nullable `exhibit_slot text` column with a SQL `CHECK` constraint that allows only the five Group-A slot ids (Benchmark, Waterfall, ROI Bridge are rejected at the SQL boundary); backfills the five canonical sections (`executive_summary` → `executive_summary_portfolio`, `priority_recommendations` → `findings_risk_priority`, `readiness_assessment` → `diagnostic_capability_maturity`, `stakeholder_synthesis` → `diagnostic_stakeholder_coverage`, `roadmap` → `roadmap_90_day_sequence`); and is idempotent on re-run. Rollback is documented inline in the migration comments. [`lib/reports/slot-map.ts`](../../lib/reports/slot-map.ts) gains `isGroupAReportSlot` (type predicate), `assertGroupAReportSlot` (lenient cast), `DEFAULT_SLOT_BY_SECTION_TYPE`, `defaultSlotForSectionType`, and `reportSlotDefinitionFor` — these are the application-side guardrails reused by mappers, the initialization action, and the renderer. New reports initialized after the migration receive their default slots automatically via `initializeReportForEngagement`. The static `GROUP_A_REPORT_SLOTS` table is no longer the render driver — it becomes the description/title registry, with the rendered slot list flowing in from `report.sections[].exhibitSlot`. When a report has zero persisted slots, the renderer shows an explicit "No persisted exhibit slots configured" notice rather than silently falling back to the static table.
