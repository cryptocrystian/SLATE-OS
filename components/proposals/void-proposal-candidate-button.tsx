...
/**
 * Phase 1B Proposal/SOW Delivery Sprint P3 — operator-only void
 * affordance for a proposal delivery snapshot. Two-step confirmation:
 *
 *   1. Click "Void" → reveals an inline form with a default reason
 *      that the operator can edit.
 ...
 * approved badge after being voided). The action also emits a
 * `proposal_snapshot_voided` activity event. After success the
 * parent page revalidates and the panel re-renders with the snapshot
 * marked voided.
 *
 * Cascade-revoke for proposal share tokens lands in Sprint P4 when
 * `proposal_share_tokens` exists; today there are no tokens to
 * cascade.
 */
export interface VoidProposalCandidateButtonProps {
  snapshotId: string;
  disabled?: boolean;
}
...
export function VoidProposalCandidateButton({
  snapshotId,
  disabled,
}: VoidProposalCandidateButtonProps) {
