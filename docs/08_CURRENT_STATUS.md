...
**Sprint 4D-A — controlled client-delivery / share canon (`docs/22`)** per `docs/19` § Proposed Sprint Sequence. Sprint 4C-D closed the acceptance backlog from `docs/21` (approved/final no-watermark + void path live-verified; Past Candidates panel + stale-acceptance UX shipped; reviewer-note opt-in and banned-phrase live test deferred with documented reasons). The next canonical step is the share-token / client-delivery boundary canon — docs-only, no code. Authors `docs/22_PHASE_1B_SHARE_TOKEN_CANON.md` (or equivalent) defining: token format · expiry · revocation · audit trail · no-indexing · proxy layer · audience model · interaction with the existing operator-only snapshot pipeline. `Send to Client`, `Prepare SOW Draft`, `Prepare Client Review`, `Prepare Report` remain locked throughout Sprint 4D-A. After 4D-A is accepted, the canonical sequence per `docs/19` § Proposed Sprint Sequence resumes (Sprint 4D implementation → separate Proposal/SOW canon → separate Benchmark Gate 1 / Financial Gate 1 → explicitly later e-signature / CRM).

Sprint 4C-D polish backlog (carried forward to Sprint 4D / 4E):
...
3. Stale-acceptance live test (no naturally stale slots in the current engagement; revisit when stale data appears naturally or with explicit operator approval to back-date a row)
4. Cross-browser pagination capture (Firefox / Edge — only Chrome verified via Chrome MCP this sprint)
...
The original Sprint 4C-D Recommended Next Step (operator-driven acceptance pass) is now satisfied except for the items above: Sprint 4C-C audit (above) accepted Sprint 4C-B with notes; the deferred items it surfaced are operator-driven acceptance checks that need a stable dev server to complete:

1. **Live banned-phrase claim-guard test** — exercise the rejection path on a stable dev server (UI-driven phrase insertion → Generate PDF Candidate → verify `claim-guard-violation` + no snapshot row + `report_pdf_candidate_failed` activity event → revert).
2. **Live approved/final R2 test** — promote a section via the existing `Approve section` action bar, regenerate, verify `draft_watermark=false`.
3. **Live void test** — add a small "Past candidates" UI panel with a void button, verify `voidReportDeliverySnapshotAction` flips `status='voided'` + emits `report_delivery_snapshot_voided`.
4. **Stale-data per-slot acceptance UX** — checkbox checklist on the Generate PDF Candidate flow + extend action signature to thread `acceptedStaleSlots: ReportExhibitSlot[]` end-to-end.
5. **Reviewer-note opt-in toggle** — optional include of `reviewerNote` in `section_snapshot[]` (and therefore in the claim-guard scan).
6. **Voided-snapshot history panel** — list past snapshots per report; show void status + reason.
7. **Cross-browser pagination check** — capture print dialogs on Chrome / Firefox / Edge to lock down the Save-as-PDF visual contract before Sprint 4D-A authorizes any client-delivery surface.

Sprint 4C-D is operator-only. **No locked client-facing controls are unlocked** by 4C-D. `Send to Client`, `Prepare SOW Draft`, `Prepare Client Review`, `Prepare Report` remain locked verbatim.

After Sprint 4C-D, the canonical sequence per `docs/19` § Proposed Sprint Sequence resumes:

1. **Sprint 4D-A — controlled client-facing delivery / share canon (docs-only — `docs/22`).** Authors the share-token + CRM-integration boundary before any code lands. Required before any Sprint 4D implementation begins.
2. **Sprint 4D — controlled client-facing delivery / share, if approved.** Either a controlled share-link surface (token + expiry + revocation + audit + no-index) or operator-mediated CRM / email delivery; unlocks `Send to Client` and possibly `Prepare Report` only when R3 is satisfied.
3. **Separate sprint — Proposal / SOW delivery canon.** Authors a future proposal-delivery canon doc. Required before any proposal-side `LockedActionButton` is unlocked.
4. **Separate sprint — Benchmark Gate 1.** Advances `docs/14` to `internal_directional`; required before Benchmark Comparison Bars can appear in any client-bound artifact.
5. **Separate sprint — Financial Gate 1.** Advances `docs/15` to `operator_estimated`; required before AI-Savings Waterfall and ROI Bridge can appear in any client-bound artifact.
6. **Separate sprint, explicitly later — e-signature / CRM integration.** Out of scope until at least Sprint 4D and the proposal-delivery canon land.

E-signature, CRM integration, and BuildOps remain explicit non-goals at the current boundary. Per `docs/19` § Locked Control Policy, `Send to Client` is the **last** control unlocked, not the first. The Visx plan file at `~/.claude/plans/which-charts-library-is-replicated-adleman.md` remains parked.
