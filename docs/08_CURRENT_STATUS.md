...
**Phase 1B Report Exhibit Wiring Sprint 1 — internal report-slot rendering** (per the staged sequence locked in `docs/17`). Sprint 0B landed the adapter layer and diagnostic surface; Sprint 1 is the first sprint that touches the report builder. It explicitly stops short of PDF export, client-facing "Send to client" / "Share" actions, and Group B (Benchmark / Waterfall / ROI Bridge) wiring.

The sprint should:

1. Parameterize the proof-of-fit `ExecutiveSummaryTwoByTwo` exhibit (it is the only Group-A exhibit not yet parameterized) so it accepts the `ExecutiveSummaryPortfolioProps` shape the Sprint 0B adapter already returns. Lift the existing internal `SAMPLE_DATA` into the preview page so the preview surface is unchanged.
2. Define how `report_sections.body` references a `ReportExhibitSlot`. Options enumerated in `docs/17` § Report Wiring Sequence Sprint 2 should be considered here at a small scale — likely a thin section-side mapping in `lib/reports/slot-map.ts` is enough to land Sprint 1 without changing `report_sections` schema.
3. Wire Group-A exhibits into the internal report preview at `/app/engagements/[id]/report` by reading the slot reference and mounting the adapter + exhibit. Use the per-slot fallback rules from `docs/17` § Fallback Rules for empty / insufficient-data states — never fall back to sample data.
4. Continue to omit Group B from any report-section path. Continue to omit PDF / export and client-facing send / share buttons (they stay behind `LockedActionButton`).
