...
**Sprint 4D-B — `report_share_tokens` migration + token service + operator-only Generate Share Link action** per `docs/22` § Proposed Sprint Sequence — conditional on operator approval of the ten open decisions enumerated in `docs/22` § Open Decisions / Operator Approval Needed (the canon's recommended defaults stand committed unless overridden in the Sprint 4D-B prompt).

Sprint 4D-B scope per `docs/22`:
1. **Migration `0014_report_share_tokens.sql`** — table with workspace/engagement/report/snapshot FKs + `token_hash` unique + `status` CHECK constraint + `audience_label` + optional `recipient_email_hash` + `expires_at` NOT NULL + soft-revoke columns + `access_count` + `last_accessed_at` + `metadata` jsonb + 4 indexes + workspace-scoped operator-full RLS + self-contained `updated_at` trigger. Idempotent. Applied to live Supabase via the existing helper.
2. **`lib/reports/share-token-types.ts`** — TS shapes mirroring the table + the canonical `ShareTokenStatus` / `ShareTokenAudience` / etc. unions.
3. **`lib/reports/share-token-mappers.ts`** + **`share-token-queries.ts`** — DB ↔ TS mappers with defensive jsonb validation + read queries (`getActiveShareTokensForSnapshot`, `getShareTokenById`, etc.). Service-role / operator-cookie split per the existing public-vs-operator pattern.
4. **`lib/reports/share-token-service.ts`** — pure-function helpers: `generateRawToken()` (`crypto.randomBytes(32)` → base64url), `hashToken()` (SHA-256), eligibility evaluator that re-runs the full `docs/22` § Snapshot Eligibility Rules against a snapshot row.
5. **`lib/reports/share-token-actions.ts`** — `generateShareLinkAction({ snapshotId, expiresInDays?, audienceLabel?, recipientEmail? })` server action. Validates UUID + auth. Reads the snapshot. Evaluates eligibility. Generates the raw token + hash. Inserts the `report_share_tokens` row. Logs `report_share_token_created` activity event. **Returns the raw token in the action result so the operator can copy it.** SLATE never persists raw tokens. (`revokeShareTokenAction` ships alongside but the operator UI for it lands in Sprint 4D-D.)
6. **`components/reports/generate-share-link-button.tsx`** — operator-only client trigger surfaced in the Past Candidates panel for share-eligible snapshots only. Two-step copy-once UI: click → reveals raw URL + clipboard-copy button + expiry summary + audience-label optional input + recipient-email optional input → click "Create link" → POSTs the action. Success notice shows the URL one final time + warns "this URL will not be shown again."
7. **Activity event types** — add four to `lib/activity/types.ts` + tone/labels in `components/activity/activity-timeline.tsx` (`report_share_token_created` · `report_share_token_revoked` · `report_share_token_accessed` · `report_share_token_expired`).
8. **Past Candidates panel extension** — add `share-eligible` chip per snapshot row + the new `<GenerateShareLinkButton>` next to the existing `Open candidate` + `Void` buttons (only when eligibility passes).
9. **Docs / status updates** — Sprint 4D-B landing in `docs/08` + `docs/10` + walkthrough notes under `artifacts/walkthroughs/sprint-4d-b-share-token-service/notes.md`.

**Sprint 4D-B remains operator-only.** No public route. No client delivery. `Send to Client`, `Prepare SOW Draft`, `Prepare Client Review`, `Prepare Report` stay locked verbatim. The action generates a URL that operators copy externally; SLATE does not email or send anything.

After Sprint 4D-B, the canonical sequence per `docs/22` § Proposed Sprint Sequence resumes:

1. **Sprint 4D-C — public read-only `/r/[token]` route.** Server-side rendering from sanitized snapshot payload. `noindex,nofollow` + `Cache-Control: no-store`. Access logging via `report_share_token_accessed`. Rate-limiting. Eligibility re-check at render. Generic rejection page across every failure mode.
2. **Sprint 4D-D — share management UI panel.** List active tokens per engagement, show `access_count` + `last_accessed_at`, expose Revoke action. Void-cascade revoke wiring.
3. **Sprint 4D-E (optional) — Send to Client action.** Operator-mediated send via the operator's own channel OR a controlled email / CRM integration (subject to a separate channel canon). Unlocks the `Send to Client` `LockedActionButton`.
4. **Separate sprint — Proposal / SOW delivery canon.** Authors a future proposal-delivery canon doc. Required before any proposal-side `LockedActionButton` is unlocked.
5. **Separate sprint — Benchmark Gate 1.** Advances `docs/14` to `internal_directional`; required before Benchmark Comparison Bars can appear in any client-bound artifact.
6. **Separate sprint — Financial Gate 1.** Advances `docs/15` to `operator_estimated`; required before AI-Savings Waterfall and ROI Bridge can appear in any client-bound artifact.
7. **Separate sprint, explicitly later — e-signature / CRM integration.** Out of scope until at least Sprint 4D-E and the proposal-delivery canon land.

Sprint 4C-D polish backlog (carried forward — does not block Sprint 4D-B):
