...
**Sprint 4C — client-safe report PDF candidate** per `docs/19` § Proposed Sprint Sequence. The DB hygiene micro-step is complete (orphan empty Sapient Digital `reports` row deleted; report re-initialized; all 5 canonical Group-A `exhibit_slot` values populated live; docs/18 Step 3-A live-verified at completion in 1.55 s with `exhibit_slot` preservation, partial-field UPDATE preservation, and `ai_report_section_drafted` activity event). Sprint 4B is complete (operator-only light/print palette landed via `.slate-print-light` CSS-variable scope; print route renders light-theme on screen and in print; default dark behavior preserved everywhere else). Sprint 4C is the next-authorized implementation step.

Sprint 4C scope per `docs/19` § Report Readiness Gates:
- Server-side render pipeline that produces a PDF binary intended for **operator-only** distribution (the operator generates the artifact and delivers to the client through their own channel — no SLATE-mediated send / share).
- `report_delivery_snapshots` future entity recording report_id + engagement_id + per-section updated_at + adapter sourceSummary + claim-guard scan result + operator id at generation time.
- R2 readiness gate enforced: report at `approved` or `final` status (or explicit operator override carrying a visible "draft" marker on the artifact); stale-data warnings resolved or accepted; Group-B excluded.
- Export-time rerun of `lib/ai/claim-guard.ts` (26 patterns across financial / commercial-finality / roadmap-commitment) against the snapshotted content, because operator edits between synthesis and export bypass the synthesis-time scan.
- **Unlocks `Export Report` only.** `Send to Client`, `Prepare SOW Draft`, `Prepare Client Review`, and `Prepare Report` remain locked until R3 + Sprint 4D + a separate share-token canon land.

Sprint 4C is gated on (a) operator authorization to begin the implementation, (b) a decision between native browser print-to-PDF capture (no new dep; lower fidelity) vs `@react-pdf/renderer` / `puppeteer` / `playwright` integration (new dep; higher fidelity, separate canon needed for the dependency choice), and (c) the snapshot-schema decision (new migration vs encode-in-JSON column of an existing table).
