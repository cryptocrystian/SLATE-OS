...
**Phase 1B Report Exhibit Wiring Sprint 2 — report-section model / constrained rich-text + exhibit references** (per the staged sequence locked in `docs/17`). Sprint 1 landed the internal report-slot renderer using a static `lib/reports/slot-map.ts` table; Sprint 2 owns the persisted reference model so a `report_sections` row can name a slot. Sprint 2 still does not authorize PDF / export or client-facing send / share.

The sprint should:

1. Decide between the three options enumerated in `docs/17` § Report Wiring Sequence Sprint 2: (a) a constrained block model with an `exhibit_slot` block type on `report_sections.body`; (b) a sibling `report_section_exhibits` join table; (c) inline JSON on `body` with a typed `exhibit_slot` field. Pick one with operator review before any schema change lands.
2. Migrate the chosen model (if schema is involved) behind a feature flag or with a clean rollback path. The existing 12-section advisory report template must remain intact.
3. Wire the report page to read the persisted slot reference instead of the static `GROUP_A_REPORT_SLOTS` table — that table becomes the validation source for legal slot ids, not the render driver. The static table stays for now.
4. Keep PDF / export, "Send to client" / "Share", and Group B wiring out of scope. They remain Sprint 3+ work, gated on canon advancement for the financial / benchmark exhibits.
5. Optional but recommended scope-creep guards: add a `lib/reports/slot-map.ts` `isGroupAReportSlot` predicate Sprint 2 can use to reject Group-B slot values at the persistence boundary.
