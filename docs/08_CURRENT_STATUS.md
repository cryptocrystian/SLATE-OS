...
**DB hygiene micro-step — orphan empty `reports` row cleanup for Sapient Digital.** This is the single shortest path to unblocking the `docs/18` Step 3 happy-path live verification. The orphan empty `reports` row at engagement `76097653-fedb-42e5-9ef6-e89a0e97f802` (created during a pre-migration init attempt before `0012_report_section_exhibit_slot.sql` was applied) has 0 sections; `initializeReportForEngagement` is idempotent on report existence and returns early, so the operator-side `Initialize report` action cannot re-seed sections without first removing the orphan row. Cleanup SQL (operator-authorized, not yet run) is staged in `artifacts/walkthroughs/sprint-3-print-preview/blocker-fix-notes.md`. After cleanup, re-run the docs/18 Step 3-A case (Generate AI draft on an editable section) to confirm the report-section AI happy path live-passes; this leaves Steps 3 / 4 / 5 fully live-verified.

After the micro-step, the canonical sequence per `docs/19_PHASE_1B_CLIENT_DELIVERY_CANON.md` is:

1. **Sprint 4B — internal print polish + light-theme Group-A palette.** Operator-only; no client-facing surface unlocked. Group-A exhibits gain a print-safe light-theme palette + a light variant of `report-print-document.tsx`. Group-B remains untouched. No locked control unlocked. **This is the prerequisite for Sprint 4C; it must land first.**
2. **Sprint 4C — client-safe report PDF candidate.** Operator-only generation; snapshot model implemented; R2 readiness gate enforced; claim-guard rerun at export; Group-B still excluded; unlocks `Export Report` only.
3. **Sprint 4D — controlled client-facing delivery / share, if approved.** Either a controlled share-link surface (token + expiry + revocation + audit + no-index) or operator-mediated CRM / email delivery; unlocks `Send to Client` and possibly `Prepare Report` only when R3 is satisfied. Requires a separate share-token canon (or CRM-integration canon) authored before this sprint begins.
4. **Separate sprint — Proposal / SOW delivery canon.** Authors `docs/20_PHASE_1B_PROPOSAL_DELIVERY_CANON.md`. Required before any proposal-side `LockedActionButton` is unlocked.
...
7. **Separate sprint, explicitly later — e-signature / CRM integration.** Out of scope until at least Sprint 4D and the proposal-delivery canon land.
