...
**Sprint 4C-C — acceptance audit + visual walkthrough of the candidate route.** Sprint 4C-B implementation has landed and live-verified end-to-end on the Sapient Digital persisted UUID engagement; the recommended next step is an operator-driven acceptance pass covering (a) banned-claim rejection — manually insert a `guaranteed savings` / `payback` / `ready for signature` phrase into a section's `summary` or `draftPreview`, regenerate, verify the action returns `claim-guard-violation` with the correct `{field, code}` violation list and no snapshot row is created; (b) approved/final readiness — promote sections via the existing `Approve section` / `Lock as final` controls, regenerate, verify `draft_watermark=false`; (c) voided-snapshot path — invoke `voidReportDeliverySnapshotAction` on an earlier snapshot, verify the activity event fires and the row flips to `status='voided'`; (d) page-break behavior under browser Save-as-PDF (cross-browser sanity check on Chrome / Firefox / Edge). Once acceptance passes, the canonical next sprint sequence becomes:

**Sprint 4C-B — client-safe report PDF candidate implementation** per `docs/20_PHASE_1B_REPORT_PDF_CANDIDATE_CANON.md` § Sprint 4C Implementation Plan, if and when the operator approves the nine open decisions enumerated in `docs/20` § Open Decisions / Operator Approval Needed. Sprint 4C-A (this sprint) is the decision canon; Sprint 4C-B is the recommended single-sprint implementation path when Option A (native browser print-to-PDF + metadata-only snapshot, no new dependency) is approved.

Sprint 4C-B scope per `docs/20`:
- Migration `0013_report_delivery_snapshots.sql` creating the canonized table + RLS policy mirroring the existing operator-full pattern (idempotent).
- `lib/reports/snapshot-types.ts` + `snapshot-queries.ts` + `snapshot-actions.ts` (`generateReportPdfCandidate` + `voidReportPdfCandidateSnapshot`).
- `lib/reports/r2-readiness.ts` pure-function helper returning `{ ready, blockers }`.
- `lib/reports/export-claim-guard.ts` pure-function helper that reuses `lib/ai/claim-guard.ts`'s shared 26-pattern scanner.
- `app/app/engagements/[id]/report/pdf-candidate/page.tsx` operator-only candidate route reusing the Sprint 4B `.slate-print-light` palette via the existing CSS-variable scope; no new exhibit / primitive / adapter code.
- `components/reports/generate-pdf-candidate-button.tsx` replacing `LockedActionButton label="Export Report"` with an active `Generate PDF Candidate` action. **`Send to Client` / `Prepare SOW Draft` / `Prepare Client Review` / `Prepare Report` stay locked verbatim.**
- New activity event types in `lib/activity/types.ts` + `components/activity/activity-timeline.tsx` (`report_pdf_candidate_generated` · `report_pdf_candidate_failed` · `report_pdf_candidate_downloaded` · `report_delivery_snapshot_voided`).
- Walkthrough notes at `artifacts/walkthroughs/sprint-4c-pdf-candidate/notes.md` once implementation lands.

**If any of the nine open decisions in `docs/20` are not resolved at acceptance time** — for example if the operator wants to evaluate Option B (server-side PDF library) before approving — split into Sprint 4C-B dependency canon (docs-only — `docs/21_PHASE_1B_PDF_LIBRARY_DEPENDENCY_CANON.md`), Sprint 4C-C snapshot schema + R2 readiness (code, no PDF binary), and Sprint 4C-D generation action + artifact route (code).

After Sprint 4C, the remaining canonical sequence is:
1. **Sprint 4D — controlled client-facing delivery / share, if approved.** Either a controlled share-link surface (token + expiry + revocation + audit + no-index) or operator-mediated CRM / email delivery; unlocks `Send to Client` and possibly `Prepare Report` only when R3 is satisfied. Requires a separate share-token canon (or CRM-integration canon) authored before this sprint begins.
2. **Separate sprint — Proposal / SOW delivery canon.** Authors `docs/20_PHASE_1B_PROPOSAL_DELIVERY_CANON.md`. Required before any proposal-side `LockedActionButton` is unlocked.
3. **Separate sprint — Benchmark Gate 1.** Advances `docs/14` to `internal_directional`; required before Benchmark Comparison Bars can appear in any client-bound artifact.
4. **Separate sprint — Financial Gate 1.** Advances `docs/15` to `operator_estimated`; required before AI-Savings Waterfall and ROI Bridge can appear in any client-bound artifact.
5. **Separate sprint, explicitly later — e-signature / CRM integration.** Out of scope until at least Sprint 4D and the proposal-delivery canon land.
