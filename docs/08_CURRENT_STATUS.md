...
**Phase 1B Report Exhibit Wiring Sprint 0 — adapter and slot mapping** (per `docs/16` recommendation). This is the first sprint after preview-library acceptance and explicitly stops short of editing `ReportWorkspace`, `report_sections`, or any client-facing surface.

The sprint should:

1. Author `docs/17_PHASE_1B_REPORT_EXHIBIT_WIRING_CANON.md` defining the exhibit → report-section slot vocabulary, the adapter contract (signature shape, validation responsibilities, location under `lib/charts/adapters/`), the data-staleness rule, and the Group-B holding rule that keeps Benchmark Comparison Bars / AI-Savings Waterfall / ROI Bridge preview-only until `docs/14` / `docs/15` reach their next data tier.
2. Scaffold the five Group-A adapters under `lib/charts/adapters/*.ts` without yet using them inside any report-section UI. Each adapter receives the persisted shape (`opportunities` / `findings` / `stakeholder_intake_responses` / `roadmap_items`) and returns the exhibit's narrow input contract. Adapters are pure and SSR-safe.
3. Add an operator-only diagnostic surface (no nav link) that renders each Group-A adapter against a seeded persisted engagement so adapters can be visually validated before report-section integration.
