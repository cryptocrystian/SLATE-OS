...
**Phase 1B Report Exhibit Wiring Sprint 0B — adapter scaffolding + diagnostic surface** (per the staged sequence locked in `docs/17`). Sprint 0A landed the canon doc; Sprint 0B is the first implementation sprint. It explicitly stops short of editing `ReportWorkspace`, `report_sections`, or any client-facing surface.

The sprint should:

1. Create `lib/charts/adapters/types.ts` declaring the canonical `ChartAdapterResult<TProps>` envelope (`status` / `props` / `issues` / `sourceSummary`) and the `ReportExhibitSlot` union per `docs/17`.
2. Scaffold the five Group-A adapters under `lib/charts/adapters/*.ts` (`executive-summary-2x2-adapter.ts`, `risk-adjusted-priority-quadrant-adapter.ts`, `capability-maturity-heatmap-adapter.ts`, `stakeholder-coverage-matrix-adapter.ts`, `roadmap-gantt-adapter.ts`). Each is a pure function that receives already-fetched persisted rows and returns a typed adapter result. Adapters do not fetch, mutate, throw, or import React / DB / app routes.
3. Add an operator-only diagnostic surface at `app/app/engagements/[id]/chart-diagnostics/page.tsx`. Reuses existing `lib/<domain>/queries.ts` helpers — no new queries, no mutations, no PDF, no AI synthesis. Renders each Group-A exhibit side-by-side with its `ChartAdapterResult` payload for visual validation. Group B (Benchmark, Waterfall, ROI Bridge) is explicitly omitted.
