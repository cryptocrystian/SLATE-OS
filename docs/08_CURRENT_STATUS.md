...
**Phase 1B Report Exhibit Wiring Sprint 3 — internal PDF export of the operator-only report preview** is the next stage in the wiring sequence per `docs/17`. Sprint 3 produces an **operator-only "internal preview PDF"** (not yet client-facing): same five Group-A slots, same fallback rules, same Group-B omission, but rasterized for review-quality print. `Send to Client` / `Share` / SOW PDF actions remain Sprint 4+ work gated on `docs/14` / `docs/15` data-gate advancement.

Two lighter-weight follow-ups are also worth queueing as standalone sprints either before or alongside Sprint 3:

1. **AI Synthesis Step 3 — report-section drafting.** Now that `report_sections.exhibit_slot` is persisted, drafting can scope which exhibits a section legitimately embeds. The slot column is the canonical handle.
2. **Domain-shape `lastTouchedAt` surfacing.** Adapter freshness defaults to `"unknown"` everywhere today; lighting up `fresh` / `stale` badges only requires exposing the existing `updated_at` columns through the `lib/<domain>/mappers.ts` files. Small, low-risk, and unblocks more honest source-summary chrome on both the report preview and the diagnostic surface.
