...
**Phase 1B Report Exhibit Wiring Sprint 4 — light-theme palette + client-facing PDF / send / share + Benchmark Gate 1 / Financial Gate 1**, gated on (a) a successful operator walk-through against the `docs/18` test plan AND a separate Sprint-3 visual review of the internal print preview, and (b) operator approval to lift the four still-locked `LockedActionButton` controls in a controlled commercial sprint that owns SOW / e-signature / client delivery.

Two lighter-weight follow-ups worth queueing in parallel or before Sprint 4:

1. **Operator dev/staging walk-through of the `docs/18` Operator Test Plan** (17 cases across AI Synthesis Steps 3 / 4 / 5) on a persisted UUID engagement with `OPENAI_API_KEY` configured. Confirms the AI synthesis arc is live-correct before the client-facing path is unlocked.
2. **Visual review of the Sprint 3 internal preview PDF** against a real engagement, capturing the chart-rendering / page-break / banner-rendering behavior so the polish backlog is grounded in observed reality before Sprint 4 light-theme work begins.

The remaining Phase 1B sequence (downstream of Sprint 0) stays the same:

4. Add a rich-text or constrained-block document model on `report_sections.body` and render it in `ReportWorkspace`; allow sections to embed exhibits by reference (consumes the adapters from Sprint 0).
5. AI Synthesis Step 3 — report section drafting from approved findings + selected opportunities + roadmap.
6. AI Synthesis Step 4 — proposal option drafting from approved opportunities + roadmap.
7. AI Synthesis Step 5 — roadmap drafting from selected opportunities.
8. Server-only document parsing (PDF/DOCX/CSV → text) so synthesis can cite document excerpts. Storage hardening (per-asset RLS, virus scanning, content sniffing) ships alongside.
...
11. Public scorecard PDF download + emailable executive summary; advance `docs/14` to Gate 1+ with a real benchmark dataset before any benchmark exhibit reaches client-facing surfaces.
12. Advance `docs/15` toward Gate 1 (`operator_estimated`) and Gate 2 (`client_validated`) for the financial exhibits.
