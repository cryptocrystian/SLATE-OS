...
**Phase 1B Report Exhibit Wiring Sprint 3 — internal PDF export of the operator-only report preview** (per `docs/17` § Report Wiring Sequence and the `docs/18` audit recommendation). Sprint 3 produces an **operator-only "internal preview PDF"** (not yet client-facing): same five Group-A slots, same fallback rules, same Group-B omission, but rasterized for review-quality print. `Send to Client` / `Share` / SOW PDF actions remain Sprint 4+ work gated on `docs/14` / `docs/15` data-gate advancement.

Before queueing Sprint 3, run the dev/staging walk-through against the **`docs/18` Operator Test Plan** (17 cases across Steps 3 / 4 / 5) on a persisted UUID engagement with `OPENAI_API_KEY` configured. The test plan documents per-row expected DB write, `ai_synthesis_runs`, and activity-timeline behavior so the walk-through is repeatable.
