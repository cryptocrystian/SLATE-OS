...
**Context.** Step 1 (findings drafts) is complete; the natural follow-on is opportunity drafting on top of the operator-approved findings. Step 2 keeps the human-in-the-loop boundary intact in two ways: (a) only consultant-reviewed findings (`review_status in ('approved', 'report_ready')`) reach the model — needs-review/draft/rejected findings never compound unreviewed AI output, and (b) the model never sets priority or quadrant; the server derives them from the model's clamped 0–100 scores using the existing `computeQuadrant` helper, plus the existing high-risk override (`risk >= 85` forces `defer-avoid`).

**No migration.** The Step 1 `ai_synthesis_runs` table already supports arbitrary `run_type` text. Step 2 reuses it with `run_type = 'opportunity_draft'`. The `opportunities` and `opportunity_finding_links` tables from Step 7 already carry the score columns, status vocabulary, and join shape needed. No schema change ships with this step.

**Why no new model output for priority/quadrant.** The original spec called this out explicitly: trust the model only for evidence-based scoring, not for the strategic call. Two reasons. First, derivation centralization: `createOpportunity` already runs `pickQuadrant(impact, complexity, risk)` with the high-risk override, and the synthesis action shares that helper. Letting the model override quadrant would create two sources of truth for what is fundamentally one calculation. Second, operator authority: the matrix is the operator's decision surface — the AI proposes, the matrix-position quadrant decides. If the model could place an opportunity in `quick-win` directly, an operator scanning the matrix would lose the implicit guarantee that everything in `quick-win` was scored above the impact threshold and below the complexity threshold.

**Why duplicate detection is title-based, not embedding-based.** Step 2 deduplicates against existing opportunity titles (case-insensitive, whitespace-collapsed) plus dedupes within the same generated batch. Embedding-based similarity would catch paraphrases ("AI-assisted reconciliation" vs "Automated reconciliation pass") but would also require an additional API surface, additional secret material, and additional latency. Title-based dedup catches the obvious collisions (the model re-suggesting an existing opportunity verbatim) and the operator's review surface catches the rest. Embeddings can ship in a follow-on if drift becomes a real problem.

**Why the action returns `no-approved-findings` before opening a synthesis run.** The context builder short-circuits with `error: 'no-approved-findings'` when the eligible-findings query is empty. The action propagates that without inserting an `ai_synthesis_runs` row or calling the provider — the run table records actual provider invocations, not user-error pre-checks. The UI shows controlled guidance and a deep link to the findings workspace instead.

**Why `status = draft`, not `scored`.** Step 7's manual `createOpportunity` action persists with `status = 'scored'` because a human just made the strategic call. AI-drafted opportunities have model-set scores plus operator-set scores being identical (the model produced both at once), so a different signal is needed. `status = 'draft'` makes the workflow explicit: the operator must touch each opportunity (select / defer / reject, or update scores) before it counts as scored. The existing `OpportunityActionBar` handles the transitions without modification.

**Activity event vocabulary.** A new `ai_opportunities_generated` activity event type is added; failures continue to use the existing `ai_synthesis_failed` event from Step 1. Metadata is restricted to `{ runType, generatedCount, skippedDuplicateCount, provider, model }` — no opportunity descriptions, no finding statements, no stakeholder content. The Step 9 `sanitizeMetadata` continues to drop sensitive keys defensively.

**`SLATE_AI_OPPORTUNITIES_MODEL` env var added (optional).** Falls back to `SLATE_AI_FINDINGS_MODEL`, which itself falls back to `gpt-4o-mini`. The override exists because opportunity drafting may benefit from a slightly larger model in some deployments without forcing findings synthesis to pay the same cost. The build still passes with the var unset and with `OPENAI_API_KEY` unset entirely (the UI renders the controlled "AI opportunity drafting is not configured" state).

**Tradeoffs accepted.**

- **Validator drops invalid candidates rather than coercing.** A model response missing a valid `linkedFindingIds` or unknown `category` is rejected outright. We do not try to fuzzy-match the category to the closest valid value. This trades recall for safety: a synthesized opportunity with a bogus category would silently land in `Back-office Automation` and confuse the operator.
- **Linked finding IDs must come from the eligible set.** The validator cross-references `linkedFindingIds` against the actual approved/report-ready findings on the engagement; IDs not in the set are dropped. A finding can be hallucinated by the model only if it happens to produce a valid UUID that also matches an existing eligible finding — vanishingly unlikely.
- **No partial-failure rollback.** If opportunity insert succeeds but some `opportunity_finding_links` inserts fail, the opportunity stays as `draft` and the link error count is recorded on `output_summary.linkErrorCount`. The operator can attach evidence manually via the existing manual-create flow. Rolling back the opportunity insert because a link insert failed would lose work for an issue the operator can fix in seconds.
- **`status = 'failed'` if zero opportunities persist.** The action returns `ai-response-invalid` so the UI can render the standard validation-failure copy. Distinguishing "model returned empty" from "all candidates were dropped by the validator" would require a separate error code; the failure mode is the same from the operator's perspective ("no opportunities were saved").

**Validation against canon.** Step 2 introduces no new persistence beyond Step 7 + Step 1. No new RLS policies. No new provider package. The provider key never reaches the browser (`server-only` enforced). Public scorecard / intake routes are untouched. Mock slug engagements never see the AI drafting CTA (it renders inside the `isPersisted` branch).
...
**Context.** Persistence Steps 0–10 are complete; the persisted MVP arc rendered every domain table but findings remained operator-authored only. The `Finding.aiDrafted` flag was a placeholder. AI synthesis is the natural unlock — it converts the carefully gathered structured context (scorecard answers, intake responses, asset metadata) into draft findings without breaking the human-in-the-loop boundary that the canon requires.

**Provider abstraction.** `lib/ai/provider.ts` reads `SLATE_AI_PROVIDER`, `OPENAI_API_KEY`, `SLATE_AI_FINDINGS_MODEL` (default `gpt-4o-mini`). When the key is absent, `getAiProviderConfig()` returns `null` and the page renders a controlled "AI synthesis is not configured" state — the build still passes and the action returns `ai-not-configured`. No SDK dependency was added; OpenAI's Chat Completions API is small enough to call via `fetch` directly with `response_format: { type: "json_object" }` and a 60s `AbortController` timeout. Keeping zero new deps avoids tree-shake landmines and ensures the browser bundle never receives provider code.

**Why metadata-only context.** The model can reason from structured text already in SLATE — scorecard answers, intake responses, asset titles + summaries + types + status — without ever touching uploaded binary contents. We deliberately do not parse PDFs, run OCR, summarize files, or build embeddings in this sprint. Treating files as metadata-only keeps the scope tight, avoids a parsing pipeline that needs its own canon, and removes a class of leakage paths (a parsed file excerpt reaching the LLM provider). When document parsing lands as its own workstream, the synthesis prompt can be extended to include excerpts; until then, candidates that depend on a file are flagged `assumptionFlag = true`.

**Why operator-only and never auto-approved.** Every existing finding boundary (8-category vocabulary, evidence trail, reviewer note, status machine) is preserved. AI drafts are dropped into `needs_review` and the existing review-action bar is the only path to `approved` / `report_ready`. This means the AI never short-circuits the consultant's editorial role, and the report cannot ever surface an unreviewed AI draft. The CTA never appears for legacy mock slug engagements — only `loaded.kind === "real"` paths see the form.

**Why a strict allowlist validator and not raw insertion.** The validator rejects candidates with unknown categories/confidences/source types, drops malformed source refs, requires either ≥ 1 source ref or `assumptionFlag = true`, and clips long fields to DB-safe lengths. Raw model output never reaches the database. An LLM that hallucinates a category (`"workflow"` instead of `"workflow_friction"`) or invents a source-type (`"intuition"`) silently has its output rejected at the validator boundary rather than corrupting the persisted finding vocabulary.

**Why store summary metadata only on `ai_synthesis_runs`.** `input_summary` records counts (intake responses, sessions, assets, scorecard answers, existing findings, min/max bounds). `output_summary` records counts (generated, skipped duplicates, candidate, insert errors) plus provider/model labels. We do not write the prompt body, raw model response, intake answer text, file names, storage paths, or stakeholder names anywhere on the run row or in activity metadata. If a future workstream needs prompt/response replay for debugging, that goes behind a separate operator-only "synthesis trace" table with explicit retention rules — not retroactively into the existing run row.

**Why dedupe statements and not re-rank.** Statement deduplication is case-insensitive whitespace-collapsed compared to existing finding statements on the engagement. Re-ranking would require introducing a separate scoring pass and arguing about how to reconcile model confidence against operator confidence — too much surface area for a Step 1 sprint. The current behavior produces a clean `Generated N · Skipped M duplicates` UI signal and avoids re-inserting near-duplicates after multiple runs.

**Why service-role is never used here.** Synthesis is operator-only, so the cookie-bound authenticated server client suffices: RLS evaluates with `auth.uid()`, every insert/update obeys workspace-scoped policies, and the service-role key stays unused on this path. This intentionally diverges from the public-server contexts (Step 2 scorecard, Step 5 stakeholder responses, Step 10 storage) where a service-role client is the only viable option. Synthesis being operator-only is the simpler boundary and the right one for AI work that depends on operator judgment.

**Tradeoffs accepted.**

- **No prompt/response audit trail.** Future debugging of "why did the model produce X?" cannot reconstruct the exact request. We accept this for privacy + storage hygiene; if a debugging incident requires reconstruction, the structured context is reproducible from the same engagement context as long as the source data hasn't shifted.
- **No streaming.** The synthesize call is a single round-trip. Response time depends on the model and provider load; the UI shows `Synthesizing…` and the action runs to completion. Streaming would require a different transport surface and offers little UX benefit when 3–7 short findings emit in a few seconds.
- **No regenerate-one or partial regen.** Step 1 generates as a batch. Per-candidate regenerate would add complexity to the run-tracking model without a clear use case yet — operators can manually edit a draft via the existing review flow.
- **No model fallback.** A failed call returns `ai-provider-failed` / `ai-rate-limited` / `ai-timeout` to the UI; we do not silently retry on a different provider. Multi-provider support belongs to its own future workstream.
- **`gpt-4o-mini` default.** Cheap and fast for a draft surface that will always be reviewed by a human. Operators can override via `SLATE_AI_FINDINGS_MODEL` when they want a stronger model.
- **Generation runs against scorecard-only context when no intake responses exist.** The form warns "most candidates will be flagged as assumptions until intake responses land" — we accept thinner outputs over forcing operators to wait for intake to complete before any AI signal arrives.

**Validation against canon.** Persistence canon `00_PERSISTENCE_AUTH_CANON.md` reserves AI synthesis as a separate workstream from the persistence sprint and requires that AI-drafted findings always pass operator approval. Step 1 honors both: synthesis is on its own AI-numbered track (AI1), and the existing `findings_operator_full` RLS plus the `needs_review` status machine are the only path findings can take into the report.

---

## 2026-05-05 — Persistence/Auth Step 10: private Supabase Storage-backed input assets; operator downloads via short-lived signed URLs; no OCR or parsing

**Decision.** Document binaries now persist to a private `engagement-documents` Supabase Storage bucket. Stakeholders can upload supporting documents through `/intake/[token]` after token validation; operators can upload from `/app/engagements/[id]/intake`; operators can download via `/api/app/assets/[assetId]/download`, which mints a 5-minute signed URL and redirects. Public surfaces cannot list, browse, or download arbitrary files. No OCR, parsing, AI summarization, or vector embeddings were introduced.

**Context.** Step 10 of `docs/persistence/02_MIGRATION_SEQUENCE.md`. Steps 0–9 closed the entire persisted advisory motion plus the operator-only audit trail; the only piece of the canon left was binary storage. Without it, `input_assets` rows held metadata but no actual document bytes, so the synthesis trail (intake → finding → opportunity → roadmap → report → proposal) had a missing link wherever a stakeholder or operator wanted to attach a real PDF / spreadsheet / process map. Step 10 closes that link without crossing into AI parsing.

**Implementation.**
- `supabase/migrations/0010_file_storage.sql` — additive `add column if not exists` on `input_assets` for the eight storage columns (`storage_bucket`, `storage_path`, `original_filename`, `mime_type`, `size_bytes`, `uploaded_by_*`, `uploaded_at`, `download_count`, `last_downloaded_at`, `checksum_sha256`). New index on `(storage_bucket, storage_path)`. Bucket provisioned via `insert into storage.buckets … on conflict (id) do update` with `public = false`, 10 MiB limit, 9-MIME allowlist. **Intentionally adds no `storage.objects` policies** — all access flows through server-side helpers.
- `lib/assets/{limits,paths,types,public,server}.ts` — single source of truth for size/MIME validation, deterministic non-PII storage paths, public-stakeholder upload helper, operator upload + signed-download helper. All `import "server-only"` where appropriate.
- Three new Next route handlers: `/api/intake/[token]/assets` (POST), `/api/app/engagements/[id]/assets` (POST), `/api/app/assets/[assetId]/download` (GET-redirect-302).
- `components/intake/public-intake-uploads.tsx` (`"use client"`) — optional upload section in `PublicIntakeForm` between questions and submit row.
- `components/intake/operator-upload-form.tsx` + `components/intake/persisted-supporting-inputs.tsx` — operator-side upload form and a download-aware supporting-inputs panel rendered only on the persisted path. Mock slug engagements continue to render the seeded `SupportingInputsPanel`.
- `lib/activity/types.ts` + `components/activity/activity-timeline.tsx` — added `input_asset_uploaded` / `input_asset_downloaded` event types and the `input_asset` entity type with tones + labels.

**Tradeoffs.**
- *No `storage.objects` RLS policies.* Supabase Storage policy semantics on private buckets are blunt — a `to authenticated` policy would grant every signed-in operator unrestricted bucket-wide read access (the bucket has a single workspace today so the blast radius is bounded, but it would still make cross-engagement enumeration possible by guessing storage paths). We keep the bucket strictly private and force every read/write through `lib/assets/*`, where the operator boundary lives. The trade-off: when multi-workspace tenancy lands, we'll need a real object-level policy that joins back to `input_assets.workspace_id`. Tracked as a follow-up.
- *Service-role client used for storage uploads/signed URLs even on the operator path.* Same reason as above — the authenticated server client doesn't have storage grants on a fully private bucket. The operator-only `auth.getUser()` short-circuit one level up enforces the boundary; the service-role key never reaches the browser. The route handlers also `revalidatePath` the operator surfaces so refresh isn't required.
- *Filenames sanitized to `[A-Za-z0-9._-]`, lowercased, capped at 96 chars.* Loses unicode / spaces / punctuation in storage paths but preserves the original filename in `original_filename` for download. Trade-off favors path-traversal safety and storage-layer compatibility over preserving exact filenames in the bucket. The operator UI displays the original filename via `original_filename`, so the user-visible name is preserved.
- *5-minute signed URL TTL.* Long enough for the operator to start a download even on slow networks; short enough that a leaked URL becomes useless quickly. URLs are never logged.
- *Stakeholder upload returns metadata only; never a download URL.* Stakeholders cannot download anything from SLATE — not even the document they just uploaded. That's a deliberate boundary: the upload UI shows the local filename + size, but the post-upload state intentionally has no "download" affordance because surfacing one would invite a future feature creep that exposes operator-only assets.
- *MIME allowlist over content sniffing.* The browser-supplied MIME type is validated against a 9-MIME list both client-side (pre-flight) and server-side (route + bucket). We don't do magic-byte sniffing — the bucket is private and only operators see the binary, so the cost of accepting a slightly mis-declared file is bounded. A future hardening pass can add server-side magic-byte verification.
- *Stakeholder upload links to `linked_role` from the session, not by parsing file contents.* No OCR, no embeddings, no auto-classification. The `evidence_quality` defaults to `unverified` for stakeholder uploads, `adequate` for operator uploads — operators can update later via the existing `input_assets.evidence_quality` column.
- *`download_count` + `last_downloaded_at` updated best-effort.* The signed URL is returned even if the counter update fails. The operator-only RLS posture means any failure here is a transient infrastructure issue, not a security one.
- *No file deduplication or checksum.* The `checksum_sha256` column exists in the schema but is unset for now. A future hardening pass can compute it during upload and reject duplicate uploads at the same path. Today, the `asset_id` segment of the path makes accidental collisions impossible.
- *No bucket-side virus scanning.* Out of scope; the canon explicitly defers OCR / parsing / scanning. Operators are the only readers; the bucket is private; the route validates MIME + size; that's the v1 boundary.

**Boundary preserved.** Public scorecard / intake surfaces never read assets, never receive signed URLs, and cannot list bucket contents. The public stakeholder upload route is the only publicly-callable asset endpoint and it requires a valid token-hash match. Service-role usage is confined to server-only helpers; the key never appears in the browser bundle. No notifications, email, CRM integration, AI synthesis, OCR, document parsing, embeddings, or BuildOps surfaces in this step. Implementation credit, locked export CTAs, and SOW execution from earlier steps are unchanged.

**Verified.** `npm run lint` clean; `NEXT_TELEMETRY_DISABLED=1 npm run build` clean (25 routes; three new API routes); `git status --short --ignored` shows `.env.local` as `!!` (ignored, untracked); no secrets, raw intake tokens, magic-link URLs, service-role keys, signed URLs, storage paths, file contents, stakeholder PII, finding excerpts, proposal pricing, or note body text printed during this sprint. End-to-end manual exercise (apply migration → confirm bucket exists → operator uploads PDF → confirms metadata + storage → downloads via signed URL → stakeholder uploads via incognito + token → confirms the operator workspace shows the stakeholder upload → confirms public scorecard cannot reach the asset routes) deferred to a real Supabase round-trip.

---

## 2026-05-05 — Persistence/Auth Step 9: operator-only activity events + internal notes land; logging is best-effort and never blocks the primary action; public surfaces never expose notes or activity

**Decision.** `/app/leads/[id]` and `/app/engagements/[id]` now expose persisted operator-only internal notes and a per-entity activity timeline. Core operator actions across Steps 4–8 emit best-effort activity events under operator-only RLS. The public stakeholder submit path emits a single service-role activity event but never surfaces activity rows back to the stakeholder. Public scorecard routes are untouched.

**Context.** Step 9 of `docs/persistence/02_MIGRATION_SEQUENCE.md`. Steps 0–8 closed the public-scorecard → lead inbox → engagement creation → stakeholder intake → findings → opportunities → roadmap → reports → proposals chain. Without persisted activity + notes, an operator could perform any of the persisted actions and have no auditable history beyond the row itself, and the existing static `notes: string[]` arrays on engagement and lead were inert. Step 9 closes the audit gap and replaces those arrays with real persistence.

**Implementation.**
- `supabase/migrations/0009_activity_notes.sql` — two tables. `activity_events` with polymorphic `entity_type` + `entity_id`, optional `engagement_id` / `lead_id` / `account_id` / `contact_id` for fast per-entity queries, text-typed `event_type`, sanitized `metadata jsonb`. RLS: operator-only `select` plus operator-only `insert` policy. `notes` with polymorphic `entity_type` + `entity_id`, soft delete via `deleted_at`, `pinned bool`, internal-only `visibility text default 'internal'`, full operator CRUD. Both tables workspace-scoped via the existing single-workspace policy shape; no anon policies.
- `lib/activity/log.ts` (`import "server-only"`) — `logActivityEvent(input, options)` helper. Defaults to the cookie-bound authenticated server client; opt into `{ viaServiceRole: true }` for public-server contexts (intake submit). Best-effort: every error is swallowed after a sanitized `console.error`. `sanitizeMetadata` shallow-clones the caller's metadata, drops keys matching `/token|secret|password|apikey|authorization|cookie|email|body|raw|excerpt/i`, truncates strings to 200 chars, caps array length to 10, caps key count to 12.
- `lib/activity/queries.ts` (`import "server-only"`) — `getActivityForEngagement` (limit 25) + `getActivityForLead` (limit 15). Authenticated server client; `profiles` join hydrates actor display names.
- `lib/notes/{queries,actions}.ts` — `getNotesForEntity` returns non-deleted notes ordered by `pinned desc, created_at desc`. Actions: `createNote`, `updateNote`, `deleteNote` (soft delete), `toggleNotePinned`. `resolveEntityContext` validates the entity exists and pre-populates `lead_id` / `engagement_id` for fast per-entity queries. Each create/update/delete emits a `note_created` / `note_updated` / `note_deleted` activity event.
- Step 4–8 actions extended with `logActivityEvent` calls covering `engagement_created`, `lead_status_changed`, `intake_session_created`, `intake_response_submitted` (service-role), `finding_created` / `finding_approved` / `finding_rejected` / `finding_report_ready`, `opportunity_created` / `opportunity_selected` / `opportunity_deferred` / `opportunity_rejected`, `roadmap_item_created` / `roadmap_item_status_changed`, `report_initialized` / `report_section_status_changed`, `proposal_initialized` / `proposal_option_recommended` / `proposal_status_changed`.
- `components/activity/activity-timeline.tsx` — server-rendered compact timeline with typed event-tone badges (24 mapped event types), actor display name (falls back to "System"), short timestamp, title + optional summary. Raw metadata is intentionally never rendered.
- `components/notes/notes-panel.tsx` (`"use client"`) — composer with `Internal-only` label, pin/unpin, edit-in-place, soft delete; `useTransition` for pending + error feedback.
- `app/app/leads/[id]/page.tsx` swaps the static `LeadNotesPanel` for `<NotesPanel entityType="lead" />` and renders `<ActivityTimeline heading="Lead activity" />` in the sidebar.
- `app/app/engagements/[id]/page.tsx` parallel-fetches notes + activity only for UUID engagements (mock slug engagements skip both queries) and conditionally renders `<NotesPanel />` + `<ActivityTimeline />` for the persisted path; legacy slug engagements continue rendering `<EngagementNotesPanel />` from the seeded fixture.

**Tradeoffs.**
- *Best-effort logging that never blocks the primary action.* The logger swallows errors after a sanitized `console.error`. The audit trail is therefore not strictly transactional with the underlying business write. This is the canon trade-off (Step 9 — "Limit event emission to status-change events and creation events"): we'd rather lose an audit row than fail a finding-approval action because a logger insert hit a transient RLS issue.
- *Activity emitted via app-layer calls, not Postgres triggers.* The canon allowed either; we chose app-layer because (a) the metadata shape stays in TypeScript so future events remain typed, (b) we can sanitize metadata before persisting, (c) we can opt in to service-role for public-server callers (`intake_response_submitted`) without granting trigger privileges in DB-land.
- *Polymorphic notes via `entity_type` + `entity_id` text columns, not a separate table per entity.* Mirrors the canon `notes.subject_table` + `subject_id` shape. Trade-off: no FK constraint on `entity_id`, but the `resolveEntityContext` helper validates the entity exists at write time and the `entity_type` is a closed text union enforced in app code. Cascade deletes flow through `lead_id` / `engagement_id` (for lead/engagement subjects); for finding/opportunity/report/proposal/report_section subjects, the row's `engagement_id` / `lead_id` cascade keeps the note out of orphan-land when the parent engagement or lead goes away.
- *Soft delete via `deleted_at` instead of `delete`.* Notes are operator-authored content; soft delete preserves the audit trail and lets a future "restore" affordance work without a backfill. The query layer filters `deleted_at is null` and the activity event records `note_deleted` so the timeline still reflects the user's intent.
- *Metadata sanitization is defense-in-depth.* Callers are still expected to pass safe metadata, but the helper drops keys matching `/token|secret|password|apikey|authorization|cookie|email|body|raw|excerpt/i`, truncates strings, and caps key count. Accidental future leakage is contained without depending on every caller getting it right.
- *Status-change events only — no per-keystroke audit.* `note_updated` fires once per save, not per keystroke. Section-status / opportunity-status / roadmap-status events fire on the resulting status (so `markFindingNeedsReview` does not emit a dedicated event because it's a routine triage move; `approveFinding` / `rejectFinding` / `markFindingReportReady` do, because they are commitment moves). This keeps the timeline legible.
- *Activity timeline is operator-only and limited to 25 events per engagement / 15 per lead in the v1 UI.* No pagination yet; the limits map to "what fits in a sidebar before scrolling fatigue sets in." A future pagination pass can add `created_at < cursor` infinite scroll without a schema change.
- *Mock slug engagements skip notes + activity entirely.* The persisted notes/activity surfaces only render for UUID engagements. Legacy slug engagements continue rendering the static `EngagementNotesPanel` from the seeded fixture. This preserves the demo paths exactly.
- *No backfill of historical events.* Step 9 explicitly does not synthesize activity events for actions that landed before the migration. Activity starts populating from this sprint forward.

**Boundary preserved.** Notes and activity are operator-only. Public `/scorecard/*` and `/intake/[token]` cannot read either table. The public stakeholder submit path emits a single service-role activity event but never surfaces it back to the stakeholder; the response shape is unchanged. Service-role usage is confined to `lib/activity/log.ts` for explicit `viaServiceRole: true` callers; the key never appears in the browser bundle. No notifications, no email, no CRM integration, no AI synthesis, no BuildOps surfaces.

**Verified.** `npm run lint` clean; `NEXT_TELEMETRY_DISABLED=1 npm run build` clean (22 routes); `git status --short --ignored` shows `.env.local` as `!!` (ignored, untracked); no secrets, raw intake tokens, magic-link URLs, service-role keys, stakeholder PII, finding excerpts, proposal pricing, or note body text printed during this sprint. End-to-end manual exercise (apply migration → operator adds lead note → confirms `note_created` activity → edits → confirms `note_updated` → soft-deletes → confirms `note_deleted` → opens engagement → adds engagement note → approves a report section → confirms timeline reflects both events → confirms public scorecard / intake routes 200 without any activity exposure) deferred to a real Supabase round-trip.

---

## 2026-05-05 — Persistence/Auth Step 8: reports + proposals persist with traceable links; implementation credit lands as bounded commercial lever; export / SOW / send remain locked

**Decision.** `/app/engagements/[id]/report` and `/app/engagements/[id]/proposal` now read/write real `reports`, `report_sections`, three report-section join tables, `proposals`, `proposal_options`, and two proposal-option join tables for UUID engagements. Operators initialize a 12-section report and a 3-option proposal per engagement (idempotent), approve sections through the lifecycle (`not-started → drafted → needs-review → approved → final`), mark a recommended option, and edit canonical implementation-credit copy under operator-only RLS. Production export, SOW draft, send-to-client, and e-signature stay behind `LockedActionButton`; AI synthesis remains deferred. Engagement detail's Report and Proposal panels reflect live counts via two new derived overlays (`mergeReportStatus`, `mergeProposalStatus`).

**Context.** Step 8 of `docs/persistence/02_MIGRATION_SEQUENCE.md`. Steps 0–7 closed the public-scorecard → lead inbox → engagement creation → stakeholder intake → findings → opportunities → roadmap chain. Without persisted reports + proposals, the AI Opportunity Sprint deliverables exist only as seeded fixtures; a real engagement has no anchor for the commercial conversation. Step 8 closes the deliverable assembly loop without crossing into commercial execution (legal SOW, billing, e-signature, send-to-client) — those remain explicitly out of scope until a later sprint.

**Implementation.**
- `supabase/migrations/0008_reports_proposals.sql` — eight tables. Two parents (`reports` unique-on-engagement, `proposals` unique-on-engagement), two child rows (`report_sections` unique-on-(report,section_type), `proposal_options` unique-on-(proposal,option_type)), and five join tables (three for report sections → findings/opportunities/roadmap, two for proposal options → opportunities/roadmap). All tables have operator-full RLS using the existing single-workspace policy shape; no anon policies. The `proposals.recommended_option_id` FK is added in an idempotent `do $$ … end $$` block after `proposal_options` exists so a fresh install applies cleanly. Implementation-credit columns live inline on `proposals` with the canonical commercial-lever copy as default values.
- `lib/reports/{mappers,queries,actions}.ts` (server-only / `"use server"`) — `initializeReportForEngagement` is idempotent and seeds the 12 canonical sections via `SECTION_ORDER` from `lib/reports/helpers.ts`. `getReportForEngagementPersisted` joins report + sections + the three link tables and returns the existing TS `Report` shape with sections sorted by canonical order. `getReportStatusSummary` uses `count: "exact", head: true` on the link tables so we never hydrate rows we don't need. Section actions: `approveReportSection`, `markReportSectionNeedsReview`, `markReportSectionFinal`, `markReportSectionDrafted`, `updateReportSectionDraft` (auto-promotes `not_started → drafted` when content lands), `updateReportSectionNote`, plus `linkReportSection` / `unlinkReportSection` for the three link kinds (treats `23505` unique-violation as benign).
- `lib/proposals/{mappers,queries,actions}.ts` (server-only / `"use server"`) — `initializeProposalForEngagement` seeds the three canonical SOW options pre-pointing AI Workflow System as recommended. `getProposalForEngagementPersisted` joins proposal + options + the two link tables. `mapProposalRow` always synthesizes the canonical `ImplementationCredit` block (falls back to canonical copy when columns are null) so the public `ImplementationCreditPanel` keeps rendering verbatim. Status actions: `approveProposal`, `markProposalNeedsReview`, `reopenProposal`. Option actions: `markProposalOptionRecommended` (atomically demotes other options first then promotes), `updateProposalOption` (per-field), `updateImplementationCredit` (treats blank fields as a request to restore the canonical default copy), plus `linkProposalOption` / `unlinkProposalOption` for opportunities and roadmap items.
- `components/reports/report-workspace.tsx` and `components/proposals/proposal-workspace.tsx` extended with `renderActionBar?` / `renderOptionActionBar?` render-props (mirrors the Step 6 / Step 7 pattern). Mock paths render the static mock buttons; persisted paths inject the real action bars.
- New `"use client"` components: `report-section-action-bar.tsx`, `proposal-option-action-bar.tsx`, `initialize-report-form.tsx`, `initialize-proposal-form.tsx`. All use `useTransition` with inline error/saved feedback.
- `app/app/engagements/[id]/report/page.tsx` and `…/proposal/page.tsx` — branch on `loadEngagementForSubroute`. UUID engagements parallel-fetch persisted data, render the initialize CTA when no row exists, and render the persisted workspace with the action bar render-prop wired. Mock slug engagements continue rendering seeded fixtures unchanged. Meta strip toggles between "Persistence Step 8 · Live" and "Sprint 7 · Mock data". Locked CTA copy tightened from "Mock" to "Locked" so the affordance reads accurately on persisted engagements.
- `app/app/engagements/[id]/page.tsx` — `mergeReportStatus(engagement, summary)` overlays `<approved>/<total>` plus an evidence-link state line; rotates next-action copy through "Initialize the report outline → Review N sections → Draft N not-started sections → Lock approved as final → Open the proposal builder". `mergeProposalStatus(engagement, summary)` overlays option count + recommended option title; rotates next-action copy through "Initialize the proposal → Mark a recommended option → Move into review → Approve → Validate scope before quoting". Both fall back to the persisted jsonb defaults when zero rows exist.

**Tradeoffs.**
- *Idempotent initialize over auto-seeding on engagement creation.* Step 4 already persists default `report_status` / `proposal_status` jsonb, but does not create the actual report/proposal rows. Auto-seeding at engagement creation would either back-populate every legacy engagement (potentially breaking Step 7 demos) or require a tedious migration. An explicit operator initialize CTA avoids that decision and keeps Step 8 purely additive — the operator deliberately starts the deliverable assembly when ready.
- *Twelve canonical sections + three canonical options seeded at initialize time.* The vocabulary is fixed by `SECTION_ORDER` and the three canonical option types. Custom sections / custom option types are out of scope. Forward-compatible: text-typed columns (`section_type`, `option_type`, `status`, `confidence`, `export_status`) let the vocabulary grow without another migration.
- *Implementation credit inline on `proposals`, not a separate table.* The credit shape is bounded — eligibility flag + amount placeholder + window + notes. A 1-1 child table would add joins for nothing. Defaulting the columns to the canonical commercial-lever copy keeps the `ImplementationCreditPanel` text constant unless the operator deliberately edits, and clearing a field restores the default copy via `updateImplementationCredit`.
- *Status / section_type / option_type / confidence / export_status stored as `text` not `enum`.* Matches Steps 4.5 / 5 / 6 / 7. Vocabulary still evolving (e.g. a future `recommended-with-conditions` proposal status, an `outdated` section status). App code is the source of truth; mappers reject unknown values with safe defaults.
- *Report sections sorted client-side by canonical order, not by `position`.* The DB stores `position` for forward-compatibility (drag-drop sequencing), but the renderer applies `SECTION_ORDER` to keep section order canonical regardless of insert order. This means re-running initialize wouldn't reorder anything; it short-circuits to the existing report id.
- *No production export, no e-signature, no send-to-client.* Explicit canon decision. The Step 8 commercial boundary is "deliverable assembly only". `LockedActionButton` retained on `Export Report`, `Prepare Client Review`, `Prepare SOW Draft`, `Send to Client`. Locked CTA copy tightened from "Mock" to "Locked".
- *Three link tables for report sections instead of one polymorphic table.* Each report section can link to findings, opportunities, and roadmap items. A polymorphic `report_section_links` table with a `ref_type` discriminator was considered. Three typed tables won: stronger FK constraints (`on delete cascade` straight from each ref source), simpler query shapes, no need for a discriminator union, and the same pattern repeats cleanly for proposal-option links.
- *`evidenceLinks` count via `count: "exact", head: true`.* The status summary needs total link count without the rows themselves. Using `head: true` skips row hydration entirely. Three reads in parallel keep latency bounded.
- *Implementation-credit copy persisted verbatim.* The canonical commercial-lever sentence ("If the client proceeds into implementation within the agreed window, a portion of the AI Opportunity Sprint fee may be credited toward the implementation SOW. This is represented here as a commercial lever, not an automatic discount.") is the migration default and the fallback in `mapProposalRow`. Operators can edit per-engagement; clearing restores the default.

**Boundary preserved.** Reports + proposals are operator-only. Public `/intake/[token]` and `/scorecard/*` cannot reach them (no anon policies, no service-role usage in this code path). `/app/*` middleware-gated as before. No AI synthesis, no PDF export, no SOW execution, no e-signature, no payment, no email automation in this step. Mock slug engagements continue to render `MOCK_REPORTS` and `MOCK_PROPOSALS` unchanged. Implementation credit remains a bounded commercial planning lever — never an automatic discount or a legally binding term.

**Verified.** `npm run lint` clean; `NEXT_TELEMETRY_DISABLED=1 npm run build` clean (22 routes, `/report` 6.89 kB, `/proposal` 5.97 kB); `git status --short --ignored` shows `.env.local` as `!!` (ignored, untracked); no secrets, raw intake tokens, magic-link URLs, service-role keys, stakeholder PII, finding excerpts, or section drafts printed during this sprint. End-to-end manual exercise (apply migration → operator initializes report → approves a section → marks final → initializes proposal → marks a different option recommended → confirms engagement detail panel updates → confirms public route cannot reach reports/proposals) deferred to a real Supabase round-trip.
...
**Context.** Step 6 of `docs/persistence/02_MIGRATION_SEQUENCE.md`. Steps 0–5 closed the public-scorecard → lead inbox → engagement creation → stakeholder intake loop. Without persisted findings, the synthesis stage of the AI Opportunity Sprint remains unreviewable inside SLATE for any real engagement, and Step 7 (opportunity scoring) has no findings to attach opportunities to. Step 6 closes the intake → findings → opportunity loop's first hop.

**Implementation.**
- `supabase/migrations/0006_findings.sql` — two tables. `findings` with `category` / `confidence` / `review_status` as text columns (vocabulary still evolving), `assumption_flag` (bool) + `assumption_note`, `reviewer_note`, `ai_drafted` default false, `position` for stable ordering, `reviewed_by` FK + `last_reviewed_at`. `finding_source_refs` with `source_type` (`stakeholder_response` / `input_asset` / `scorecard_answer` / `consultant_note`), optional `source_id` uuid, display fields, `excerpt`, `strength`, `metadata jsonb`. RLS operator-full on both; no anon policies.
- `lib/findings/queries.ts` (server-only) — `getFindingsForEngagementPersisted(uuid)`, `getFindingByIdPersisted(uuid)`, `getFindingsStatusSummary(uuid)`, `getEvidenceCandidatesForEngagement(uuid)` returning intake-response + input-asset candidates joined to their session for stakeholder name + title.
- `lib/findings/mappers.ts` — DB↔TS translators for the five enum families (category, review_status, confidence, source_type, strength). `mapFindingRow(row, refs)` returns the existing TS `Finding` shape so every visual component (`FindingsWorkspace`, `EvidencePanel`, `FindingStatusChip`, `ConfidenceIndicator`) keeps its props unchanged.
- `lib/findings/actions.ts` (`"use server"`) — `createManualFinding`, `approveFinding`, `rejectFinding`, `markFindingReportReady`, `markFindingNeedsReview`, `updateFindingNote`, `editFinding`. Every action `supabase.auth.getUser()`-gates, sets `reviewed_by` / `last_reviewed_at`, bumps the engagement's `last_activity_at`, and `revalidatePath`s the findings route + engagement detail.
- `components/findings/findings-workspace.tsx` — extended with an optional `renderActionBar?: (finding) => React.ReactNode` prop. Mock paths fall back to the existing static action bar; persisted paths inject the real action bar from the route.
- `components/findings/review-action-bar.tsx` (new, `"use client"`) — Approve / Mark report-ready / Add note / Reopen / Reject buttons via `useTransition`, with inline reviewer-note editor.
- `components/findings/create-finding-form.tsx` (new, `"use client"`) — operator form with the seven canonical fields + an evidence multi-selector. Selected candidates persist as `finding_source_refs` rows on save.
- `app/app/engagements/[id]/findings/page.tsx` — branches on `loadEngagementForSubroute`. UUID engagements load real findings + candidates, render the create-finding form + persisted action bar; legacy slug engagements continue rendering `MOCK_FINDINGS` with the static mock action bar. Meta strip toggles between "Persistence Step 6 · Live" and "Sprint 5 · Mock data".
- `app/app/engagements/[id]/page.tsx` — `mergeFindingsStatus` overlays derived counts (total / needs-review / approved / report-ready / rejected) on the engagement's persisted `findings_status` jsonb when at least one finding exists. Approved + report-ready merge into the panel's `approved` total; status badge tone and next-action copy reflect the live triage state.

**Tradeoffs.**
- *Manual findings only; no AI synthesis.* The canon defers AI synthesis to a separate workstream. Step 6 ships the operator-side authoring + review path so the schema is exercised end-to-end before the synthesis pipeline arrives. `findings.ai_drafted` is forward-compatible — a future synthesis pass writes `ai_drafted = true` and the same review actions apply.
- *Evidence candidates joined in JS, not via PostgREST relationship inference.* `getEvidenceCandidatesForEngagement` issues two cheap reads (responses + sessions) and joins by `session_id` in JS. Avoids brittleness around relationship-name inference and keeps the query layer readable. The candidate list is capped at 200 responses + 50 input assets — enough headroom for any realistic engagement.
- *Source-ref `source_id` left nullable.* Lets a finding cite a piece of evidence purely by display string when the underlying record was deleted or never existed (consultant notes, off-system insights). The optional FK is recorded when an operator selects an existing candidate.
- *Review-status / category / confidence / source-type stored as `text` not `enum`.* Mirrors Step 4.5 / Step 5 — vocabulary still evolving (e.g. a future `ai_drafted` review state, additional categories). App code is the source of truth; mappers reject unknown values with safe defaults.
- *No `finding_review_events` audit table.* The migration sequence places activity events at Step 9. Step 6 stamps `reviewed_by` + `last_reviewed_at` so the latest reviewer is captured per finding; full event log lands later.
- *Reopen as an explicit action separate from Reject / Approve.* Operators occasionally want to push a previously-approved or rejected finding back into review (new evidence surfaced, scope changed). The `markFindingNeedsReview` action makes that one-click and avoids the UX trap of "reject to undo."
- *Renaming the existing `getFindingsForEngagement` to `getFindingsForEngagementPersisted` instead of dropping the mock helper.* Legacy slug engagements still need `MOCK_FINDINGS`; renaming the persisted query keeps both helpers callable from the route without conditional imports. Mock removal lands in Step 7+ alongside the rest of the legacy advisory mocks.

**Boundary preserved.** Findings are operator-only. Public `/intake/[token]` cannot read findings (no anon policies, no service-role usage in the findings code path). Public `/scorecard*` continues to know nothing of internal findings. `/app/*` middleware-gated as before. No real AI synthesis, no document parsing, no file upload, no opportunity scoring, no roadmap persistence, no report or proposal persistence, no BuildOps surfaces.

**Verified.** `npm run lint` clean; `NEXT_TELEMETRY_DISABLED=1 npm run build` clean; `git status --short --ignored` shows `.env.local` as `!!` (ignored, untracked); no secrets, raw intake tokens, magic-link URLs, service-role keys, or stakeholder PII printed during this sprint. End-to-end manual exercise (apply migration → operator authors finding with attached intake response → approve → mark report-ready → confirm public route still cannot reach findings) deferred to a real Supabase round-trip.
