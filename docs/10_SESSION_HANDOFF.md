...
Next planned: **Phase 1B Report Exhibit Wiring Sprint 1 — internal report-slot rendering** (per the staged sequence locked in `docs/17`). Sprint 0B landed the adapter layer + operator-only diagnostic surface; Sprint 1 is the first sprint that touches the report builder UI. It will (a) parameterize the proof-of-fit `ExecutiveSummaryTwoByTwo` (the only Group-A exhibit still rendering an internal `SAMPLE_DATA` array) so it accepts the `ExecutiveSummaryPortfolioProps` shape the Sprint 0B adapter already returns, lifting the existing sample data into the preview page so `/app/charts-preview` is unchanged; (b) define how `report_sections.body` references a `ReportExhibitSlot` — likely a thin section-side mapping in `lib/reports/slot-map.ts` is enough to land Sprint 1 without changing `report_sections` schema; (c) wire Group-A exhibits into the internal report preview at `/app/engagements/[id]/report` by mounting adapter + exhibit per slot reference, using the per-slot fallback rules in `docs/17` § Fallback Rules for empty / insufficient-data states (never falling back to sample data); (d) continue to omit Group B + continue to gate PDF / export and client-facing send / share behind `LockedActionButton`. Downstream of Sprint 1, the Phase 1B sequence remains: Sprint 2 report-section model / constrained rich-text + exhibit references → Sprint 3+ PDF / export and client-facing delivery → AI synthesis Steps 3/4/5 → server-only document parsing → public scorecard PDF download. The three Gate-0 exhibits remain preview-only until `docs/14` advances to Gate 1+ and `docs/15` advances to Gate 1+ respectively. E-signature, CRM integration, and BuildOps remain explicit non-goals.
