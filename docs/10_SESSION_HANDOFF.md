...
Next planned: **Phase 1B Report Exhibit Wiring Sprint 3 — internal PDF export of the operator-only report preview** (per the staged sequence locked in `docs/17`). Sprint 2 landed the persisted slot reference model; Sprint 3 produces an operator-only "internal preview PDF" (not yet client-facing): same five Group-A slots, same fallback rules, same Group-B omission, but rasterized for review-quality print. `Send to Client` / `Share` / SOW PDF actions remain Sprint 4+ work gated on `docs/14` / `docs/15` data-gate advancement. Two lighter-weight follow-ups are also queueable in parallel or before Sprint 3: (a) **AI Synthesis Step 3 — report-section drafting**, now that `report_sections.exhibit_slot` is persisted and can scope which exhibits a section legitimately embeds; (b) **Domain-shape `lastTouchedAt` surfacing** through the `lib/<domain>/mappers.ts` files so adapter freshness can move off `"unknown"` and light up `fresh` / `stale` badges on both the report preview and the diagnostic surface. The three Gate-0 exhibits remain preview-only until `docs/14` advances to Gate 1+ and `docs/15` advances to Gate 1+ respectively. E-signature, CRM integration, and BuildOps remain explicit non-goals.
