...
Next planned: **wire the Phase 1B preview library out of preview-only territory into real persisted reports.** All eight Phase 1B exhibits now ship behind the operator-only `/app/charts-preview` route. The remaining Phase 1B work is: rich-text section bodies on `report_sections.body` so sections can embed exhibits by reference → AI synthesis Steps 3/4/5 (report sections, proposal options, roadmap drafting) → server-only document parsing → real PDF export for reports + SOWs → public scorecard PDF download + benchmark dataset. The two Gate 0 exhibits (Sprint 5 Benchmark Comparison Bars; Sprints 6 + 7 financial exhibits) are gated on `docs/14` and `docs/15` reaching their next data tier before any wiring that touches client-facing surfaces. E-signature, CRM integration, and BuildOps remain explicit non-goals.
