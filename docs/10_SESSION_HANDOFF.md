...
Next planned: **Phase 1B Report Exhibit Wiring Sprint 0 — adapter and slot mapping** (per the `docs/16` acceptance recommendation). The sprint authors `docs/17_PHASE_1B_REPORT_EXHIBIT_WIRING_CANON.md`, scaffolds five Group-A adapters under `lib/charts/adapters/` (Executive Summary 2×2 / Risk Quadrant / Capability Maturity Heatmap / Stakeholder Coverage Matrix / Roadmap Gantt), and adds an operator-only diagnostic surface to visually validate each adapter against seeded persisted data. **It explicitly stops short of editing `ReportWorkspace`, `report_sections`, or any client-facing surface.** Downstream of Sprint 0, the Phase 1B sequence remains: rich-text / constrained-block section bodies on `report_sections.body` (consuming the new adapters) → AI synthesis Steps 3/4/5 (report sections, proposal options, roadmap drafting) → server-only document parsing → real PDF export for reports + SOWs → public scorecard PDF download. The three Gate-0 exhibits (Benchmark Comparison Bars, AI-Savings Waterfall, ROI Bridge) remain preview-only until `docs/14` advances to Gate 1+ and `docs/15` advances to Gate 1+ respectively. E-signature, CRM integration, and BuildOps remain explicit non-goals.
