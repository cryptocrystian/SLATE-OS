...
Next planned: **Phase 1B Report Exhibit Wiring Sprint 2 — report-section model / constrained rich-text + exhibit references** (per the staged sequence locked in `docs/17`). Sprint 1 landed the internal report-slot renderer using a static `lib/reports/slot-map.ts` table; Sprint 2 owns the persisted reference model so a `report_sections` row can name a slot rather than the render driver picking from a hard-coded table. The sprint will: (a) pick one of the three options from `docs/17` § Report Wiring Sequence Sprint 2 — constrained block model with an `exhibit_slot` block type on `report_sections.body`, or a sibling `report_section_exhibits` join table, or inline JSON on `body` with a typed `exhibit_slot` field — with explicit operator review before any schema change; (b) migrate behind a rollback path; (c) wire the report page to read the persisted slot reference instead of the static table, keeping the static table as the validation source for legal slot ids; (d) add a `isGroupAReportSlot` predicate to reject Group-B values at the persistence boundary. Downstream of Sprint 2: Sprint 3+ PDF / export and client-facing delivery → AI synthesis Steps 3/4/5 (report sections, proposal options, roadmap drafting) → server-only document parsing → public scorecard PDF download. The three Gate-0 exhibits remain preview-only until `docs/14` advances to Gate 1+ and `docs/15` advances to Gate 1+ respectively. E-signature, CRM integration, and BuildOps remain explicit non-goals.
