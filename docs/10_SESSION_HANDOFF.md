...
Next planned: **Phase 1B Report Exhibit Wiring Sprint 0B — adapter scaffolding + diagnostic surface** (per the staged sequence locked in `docs/17`). Sprint 0A landed the canon doc; Sprint 0B is the first implementation sprint. The sprint will (a) add `lib/charts/adapters/types.ts` declaring the canonical `ChartAdapterResult<TProps>` envelope and the `ReportExhibitSlot` union; (b) scaffold the five Group-A adapters under `lib/charts/adapters/*.ts` (`executive-summary-2x2-adapter.ts`, `risk-adjusted-priority-quadrant-adapter.ts`, `capability-maturity-heatmap-adapter.ts`, `stakeholder-coverage-matrix-adapter.ts`, `roadmap-gantt-adapter.ts`) as pure functions that never fetch / mutate / throw / import React / import the DB; (c) add the operator-only diagnostic surface at `app/app/engagements/[id]/chart-diagnostics/page.tsx` reading existing `lib/<domain>/queries.ts` helpers — no new queries, no mutations, Group-A only. **It explicitly stops short of editing `ReportWorkspace`, `report_sections`, or any client-facing surface, and Group B (Benchmark Comparison Bars, AI-Savings Waterfall, ROI Bridge) is explicitly omitted.** Downstream of Sprint 0B, the Phase 1B sequence remains: Sprint 1 internal report-slot rendering → Sprint 2 report-section model / constrained rich-text + exhibit references → Sprint 3+ PDF / export and client-facing delivery → AI synthesis Steps 3/4/5 (report sections, proposal options, roadmap drafting) → server-only document parsing → public scorecard PDF download. The three Gate-0 exhibits remain preview-only until `docs/14` advances to Gate 1+ and `docs/15` advances to Gate 1+ respectively. E-signature, CRM integration, and BuildOps remain explicit non-goals.
