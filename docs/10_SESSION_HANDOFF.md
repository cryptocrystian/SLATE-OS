...
Next planned: **Phase 1B Report Exhibit Wiring Sprint 3 — internal PDF export of the operator-only report preview** (per the staged sequence locked in `docs/17` and confirmed by the `docs/18` audit recommendation). Sprint 3 produces an operator-only "internal preview PDF" (not yet client-facing): same five Group-A slots, same fallback rules, same Group-B omission, but rasterized for review-quality print. `Send to Client` / `Share` / SOW PDF actions remain Sprint 4+ work gated on `docs/14` / `docs/15` data-gate advancement. Before Sprint 3 lands, run the **`docs/18` Operator Test Plan** in dev/staging against a persisted UUID engagement with `OPENAI_API_KEY` configured — 17 cases across AI Synthesis Steps 3 / 4 / 5 covering strong-evidence, locked-state, missing-env, mock-engagement, banned-claim, and existing-items scenarios with per-row expected DB / run / activity behavior. The three Gate-0 exhibits (Benchmark Comparison Bars, AI-Savings Waterfall, ROI Bridge) remain preview-only until `docs/14` advances to Gate 1+ and `docs/15` advances to Gate 1+ respectively. E-signature, CRM integration, and BuildOps remain explicit non-goals.
