...
- **B) If operator has offline-collected stakeholder responses already** → **Intake Workflow Product Gap Sprint** — ✅ **CANONIZED 2026-05-23 in `docs/37_SAPIENT_DIGITAL_OFFLINE_INTAKE_CANON.md`.** Canon defines 3 modes (Live / Operator-staged / Document-only), data model extensions, UI requirements, findings-synthesis readiness gate, Sapient-specific 5-step path, and I2-I6 implementation sequence. **Sprint I2 (data model + server actions) LANDED 2026-05-28** — migration `0017_offline_intake_extensions.sql`, 6 guarded server actions in `lib/intake/offline-actions.ts`, type extensions, mapper extensions, 6 new activity event types. **Sprint I3 (operator UI) is the next blocker for SLATE-side ingest.** Until I3 lands, operator may collect Sapient responses externally using the `docs/36` § 6 question packet AND hold them until I3, OR (after I2 migration is applied to deployed Supabase) backfill via Supabase Dashboard SQL editor as a bridge state.
