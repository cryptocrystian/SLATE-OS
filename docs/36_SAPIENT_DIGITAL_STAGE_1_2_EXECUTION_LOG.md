...
- **B) If operator has offline-collected stakeholder responses already** → **Intake Workflow Product Gap Sprint** — ✅ **CANONIZED 2026-05-23 in `docs/37_SAPIENT_DIGITAL_OFFLINE_INTAKE_CANON.md`.** Canon defines 3 modes (Live / Operator-staged / Document-only), data model extensions, UI requirements, findings-synthesis readiness gate, Sapient-specific 5-step path, and I2-I6 implementation sequence. **Sprint I2 (data model + server actions) LANDED 2026-05-28** — migration `0017_offline_intake_extensions.sql`, 6 guarded server actions in `lib/intake/offline-actions.ts`, type extensions, mapper extensions, 6 new activity event types. **Sprint I3 (operator UI) LANDED 2026-05-28** — 8 new components in `components/intake/` (source-type chip + response-status chip + StageOfflineStakeholderForm + OfflineResponseForm + OfflineResponseActions + OfflineIntakePanel + OfflineIntakeReadinessHint), new server-only `lib/intake/offline-queries.ts`, and `app/app/engagements/[id]/intake/page.tsx` wired. **Sprint I3 Live Walkthrough LANDED 2026-06-02** — `docs/38_SAPIENT_DIGITAL_I3_LIVE_WALKTHROUGH.md` documents full evidence. Vercel Production promoted from `a39a50c`; migration 0017 applied to deployed Supabase project `hhglrcvsmwaheikdvijw` via Supabase MCP (all columns + table + RLS + 12 CHECK constraints verified); full UI walkthrough on Sapient Digital exercised stage offline stakeholder → capture draft response (with PII-warning fired correctly on email pattern) → mark ready → void; all DB states match canon (`source_type='operator_entered'`, `client_visible=false`, `token_hash=null`, `sent_at=null`); activity-event metadata sanitized (no PII anywhere); all 17 boundary checks pass. Two test-artifact rows retained on Sapient Digital clearly labeled `I3 WALKTHROUGH TEST` (final response status `voided`); they do not advance the docs/35 § 5 readiness gate (still 1/15). Offline intake is now **operationally ready** against the deployed environment. Operator can begin capturing real Sapient Digital responses; once enough role coverage lands, `Sprint I5` (findings synthesis consuming `response_status = 'ready_for_synthesis'`) becomes unblockable. `Sprint I4` (binary document attachment behind `engagement_intake_documents.storage_path`) remains optional pending operator need.
