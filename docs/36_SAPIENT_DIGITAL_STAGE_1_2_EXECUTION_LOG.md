...
- **B) If operator has offline-collected stakeholder responses already** → **Intake Workflow Product Gap Sprint** — ✅ **CANONIZED 2026-05-23 in `docs/37_SAPIENT_DIGITAL_OFFLINE_INTAKE_CANON.md`.** Canon defines 3 modes (Live / Operator-staged / Document-only), data model extensions, UI requirements, findings-synthesis readiness gate, Sapient-specific 5-step path, and I2-I6 implementation sequence. **Sprint I2 (data model + server actions) LANDED 2026-05-28** — migration `0017_offline_intake_extensions.sql`, 6 guarded server actions in `lib/intake/offline-actions.ts`, type extensions, mapper extensions, 6 new activity event types. **Sprint I3 (operator UI) LANDED 2026-05-28** — 8 new components in `components/intake/` (source-type chip + response-status chip + StageOfflineStakeholderForm + OfflineResponseForm + OfflineResponseActions + OfflineIntakePanel + OfflineIntakeReadinessHint), new server-only `lib/intake/offline-queries.ts` (`getOfflineSessionsForEngagement`, `getEngagementIntakeDocuments`, `getOfflineIntakeReadinessSummary`), and `app/app/engagements/[id]/intake/page.tsx` wired to render the offline-intake section + readiness hint behind the existing persisted-engagement branch (live-link Mode A unchanged). Inline non-blocking PII-warning on response paste; affirmative "No message sent" / "Save without sending" chips on every offline save action. Lint + build clean. **Sapient Digital live walkthrough remains gated** on the operator applying migration 0017 to deployed Supabase + restarting the deployed env — verified via `information_schema.columns` that the new columns + `engagement_intake_documents` table are NOT present on deployed project `hhglrcvsmwaheikdvijw`. No mutation against any deployed engagement was performed in Sprint I3. Once 0017 is applied, the operator can drive the Sprint I3 UI through Sapient Digital end-to-end without further Claude-side code; **Sprint I4** (binary document upload backend) or **Sprint I5** (findings synthesis integration consuming `response_status = 'ready_for_synthesis'`) follow per `docs/37` § 7.
