...
- **Sapient Digital live walkthrough — DEFERRED:** Migration 0017 is NOT applied to the deployed Supabase project `hhglrcvsmwaheikdvijw` (verified via `information_schema.columns` lookup against `stakeholder_intake_sessions` — none of `source_type`, `entered_by`, `collected_at`, `source_confidence`, `operator_notes`, `client_visible` exist there; `engagement_intake_documents` table does not exist). The Sprint I3 source code is build-clean and ready; the live UI test against Sapient Digital remains gated on schema parity, which is a separate operator-authorized step (apply 0017 via Supabase migrations + restart the deployed env). No mutation was performed against any deployed engagement in this sprint.
