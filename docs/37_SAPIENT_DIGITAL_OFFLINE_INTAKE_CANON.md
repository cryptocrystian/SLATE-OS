...
### Sprint I3 — Operator UI for Draft Stakeholders + Offline Responses (LANDED 2026-05-28)

- **Scope as implemented:** Eight new client / server components under `components/intake/`:
  - `intake-source-type-chip.tsx` — reusable tone-coded chip (live_link / operator_entered / meeting_notes / transcript / email_paste / document_upload / external_link) with operator-facing tooltip.
  - `intake-response-status-chip.tsx` — lifecycle chip (draft / ready_for_synthesis / superseded / voided) with dot prefix + tooltip.
  - `stage-offline-stakeholder-form.tsx` — client form posting to `createOfflineStakeholderIntakeSessionAction` with affirmative "No message sent" / "Save without sending" chips above the submit button and an inline ShieldOff boundary reminder. Role / source-type / source-confidence / collected-at / operator-notes fields with helper copy per source type.
  - `offline-response-form.tsx` — per-question response capture form posting to `createOfflineStakeholderResponseAction`. PII-warning is inline + non-blocking, firing on regex match against email / SSN-shaped / phone / 13-16 digit numeric (credit-card-shaped) patterns; the warning recommends scrubbing but does not block save.
  - `offline-response-actions.tsx` — Mark ready / Void buttons calling `markStakeholderResponseReadyForSynthesisAction` + `voidStakeholderResponseAction`. Void uses a `window.prompt` for the optional reason (max 500 chars). Terminal statuses (superseded / voided) surface no actions per the docs/37 § 4 transition matrix.
  - `offline-intake-panel.tsx` — orchestrator showing each offline session as an expandable row with role badge, source-type chip, source-confidence chip, response counts (`N ready`, `N draft`). Each expanded session lists its responses (status chip + source chip + answer text + operator notes + actions) and exposes an inline `<details>` "Capture another response" form.
  - `offline-intake-readiness-hint.tsx` — operator-only aggregate card showing offline stakeholder count, sessions-with-ready-response count, draft/ready/superseded/voided counts, and document count, plus a synthesis-gate reminder ("Drafts and voided responses are skipped").
- **Read path:** New `lib/intake/offline-queries.ts` server-only module exposes `getOfflineSessionsForEngagement`, `getEngagementIntakeDocuments`, and `getOfflineIntakeReadinessSummary`. Live-link Mode A sessions (`source_type = 'live_link'`) are excluded from the offline panel — they continue to surface through the existing `getIntakeRecordForEngagement` + `StakeholderList` path, unchanged.
- **Wiring:** `app/app/engagements/[id]/intake/page.tsx` adds an "Offline intake" section in the main column (between the live-link CreateStakeholderForm and the existing RoleCoverageMap) rendering the StageOfflineStakeholderForm + OfflineIntakePanel, and adds the OfflineIntakeReadinessHint to the engagement aside. The persisted-engagement branch fetches `getOfflineSessionsForEngagement`, `getEngagementIntakeDocuments`, and `getOfflineIntakeReadinessSummary` in the same `Promise.all` as the existing intake fetches.
- **Source-tree fix:** Removed a stray `export { mapEngagementIntakeDocumentRow } from "./mappers"` re-export from `lib/intake/offline-actions.ts` that compiled in dev but failed `next build` because a `"use server"` file may only export async functions. Consumers (`lib/intake/offline-queries.ts`) now import the helper directly from `./mappers`. No behavioral change.
- **What didn't ship in I3 (deferred to I4-I6):** No document binary upload backend (operator can paste content_text or external_url, but no file-upload UI — Sprint I4). No findings synthesis integration (Sprint I5). No `client_visible` elevation surface (kept structurally false, docs/35 § 5 readiness gate governs). No live Sapient Digital walkthrough — see § 6 note below.
- **Boundary preserved:** Zero new public routes. Zero new package dependencies. Zero `mailto:` / `sendgrid` / `nodemailer` / `docusign` / `hellosign` / `adobesign` / `crm_push` references. No `service_role` writes anywhere — every fetch and mutation routes through the cookie-bound `createSupabaseServerClient`. No live-link Mode A code path touched. Group B contracts unchanged. The send-to-client disclaimer pin script still passes (`npm run check:send-to-client-disclaimers`). Activity events emit only safe metadata (no `answer_text`, no `content_text`, no PII).
- **Verification:** `npm run lint` clean ✅; `NEXT_TELEMETRY_DISABLED=1 npm run build` clean ✅. Intake route grew from ~9 KB to 14.2 KB First Load JS shell-side (driven by the new client components in the operator surface). All other 28 routes unchanged byte-identical. `npm run check:send-to-client-disclaimers` ✅.
- **Sapient Digital live walkthrough — LANDED 2026-06-02.** See `docs/38_SAPIENT_DIGITAL_I3_LIVE_WALKTHROUGH.md` for full evidence. Summary: (1) staging-branch Vercel Production deployment promoted from `a39a50c` (canonical `https://slate-os-staging.vercel.app` alias re-pointed to new deployment `dpl_9wFmhoX4K5FcLkzxxPiUMuSUwTZT`; post-deploy curl battery confirms `/r/test-noop` + `/p/test-noop` serve canon `no-store + noindex + no-referrer` headers; `/s/test` + `/sow/test` stay 404); (2) migration 0017 applied to deployed Supabase via Supabase MCP `apply_migration` (success, idempotent — all 7 new columns on `stakeholder_intake_sessions`, 7 on `stakeholder_responses`, 20-column `engagement_intake_documents` table with RLS authenticated-only policy, 12 CHECK constraints including the canonical `token_hash` live-vs-offline mode gate); (3) full UI walkthrough against Sapient Digital exercised stage offline stakeholder → capture draft response → PII-warning fires correctly on `test@example.com` pattern → mark ready for synthesis → void with audit reason; all four DB states match canon (`source_type='operator_entered'`, `client_visible=false` throughout, `token_hash=null`, `sent_at=null`, response_status transitions `draft → ready_for_synthesis → voided`); (4) activity event metadata fully sanitized (no `answer_text`, no `stakeholder_name`, no `email`, no operator notes content — only safe semantic fields `role`, `sourceType`, `sourceConfidence`, `sessionId`, `questionId`, `responseStatus`, `priorStatus`, `reason`); (5) all 17 boundary checks pass (zero new share tokens, zero snapshots, zero Send to Client, zero SOW events, zero documents, deployed canon routes intact, readiness gate unmoved). Two test-artifact DB rows retained on Sapient Digital (`session 91fea09e-…`, `response 02aea7f1-…` final status `voided`) clearly labeled `I3 WALKTHROUGH TEST` for audit-trail preservation; they do not contribute to the docs/35 § 5 readiness gate (still 1/15). One cosmetic finding documented in `docs/38` § 4.6: the existing live-link role-coverage map + risk derivations don't branch on `source_type`, so an offline-staged stakeholder still triggers the "1 stakeholder have not opened the intake link" risk note — non-blocking, future polish.
