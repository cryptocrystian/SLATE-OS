...
- **Follow-on (2026-06-04 — S4–S6 Walkthrough · Part 1):** S4 audit-label exclusion heuristic now **verified live on deployed Production** — see `docs/46_S4_S6_CONTROLLED_WALKTHROUGH.md`. Vercel Production was promoted from staging head `12d6fbd`; canonical alias now serves S6 code. The deployed findings page on SLATE Pilot Test Client renders the evidence panel with literal "8 test-labeled responses excluded from synthesis (audit fixture data)" — matching the 8 known historical audit-labelled responses (7 × S1 AUDIT + 1 × S2 AUDIT FIXTURE) exactly. The exclusion behavior described in this canon is therefore verified on deployed Production, not just in source-tree smokes. The live AI synthesis exercise itself is deferred to Part 2 of the walkthrough sprint.
- **Follow-on (2026-06-04 — S4–S6 Walkthrough · Part 2):** ⚠ **Live AI synthesis run verified end-to-end** against 15 transcript-source non-audit responses across 3 REQUIRED roles — see `docs/46` § 17. Gate cleared green **without operator override** (`overrideApplied: false` verified live via SQL on `activity_events`). 5 findings landed in `public.findings` as `review_status='needs_review'`, with correctly attributed source refs (strong strength, role-tagged stakeholder labels, 121–297 char excerpts). Activity-event metadata sanitization confirmed live: no raw text, no PII, no stakeholder identifiers, no transcript content; only safe semantic fields (`runType, generatedCount, skippedDuplicateCount, provider, model, overrideApplied`). **Spec drift documented:** the deployed metadata shape lacks the `evidenceLanes: {liveLink, transcript, offlineOperator, crmLinked}` field that this canon (§ 8.2) mandates. The boundary still holds (no PII), but the operator-facing lane-attribution surface is thinner than spec. Recommend including this fix in the blocker-fix sprint that addresses the post-synthesis findings-page render 500. The audit-label exclusion heuristic continued to hold throughout (still excluded the 8 historical audit rows; correctly ignored the 15 new non-audit rows).
