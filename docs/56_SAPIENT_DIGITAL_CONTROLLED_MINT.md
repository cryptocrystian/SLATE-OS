...
- **L-34 — `input_assets.deleted_at` column referenced by the loader does not exist in the deployed schema.** `lib/engagement-readiness/pre-delivery-audit-loader.ts` filters `input_assets` with `.is("deleted_at", null)`. Direct schema inspection (`information_schema.columns WHERE table_name='input_assets'`) shows no `deleted_at` column. PostgREST returns an error for the filter, which the `supabase-js` count path resolves to `null → 0`. **Effect:** `totalDocuments` always reads `0` in production, which forces C3 (`documents_not_uploaded_or_acked`) to block unless `documentsClearedOrAcknowledged === true` is explicitly passed. This is a **conservative failure mode** — it never falsely allows a mint — but the loader's contract drifts from the deployed schema. **Recommendation:** drop the `.is("deleted_at", null)` clause OR add the column via a future migration. **Severity:** low (boundary intact). **Not addressed in S12** — out of sprint scope, no source touch this sprint.
