...
> **Presentation Pass · Pass 3 follow-on note (2026-06-18):** Pass 3 ships the client content redaction layer (`lib/deliverables/client-copy-sanitizer.ts`) plus wiring through `report-pdf-candidate-document.tsx` + `proposal-candidate-document.tsx`. Closes the operator-vocabulary leak in client mode that Pass 2-Fix surfaced once the SSR crash was resolved: raw finding-UUIDs, `EVIDENCE NOTES`, `GROUP B BLOCK`, `group b canon gate`, `GROUP-B (GATED)`, `INTENTIONALLY NOT INCLUDED`, bare `*ID:` labels, and adapter-mechanics phrasing now sanitize out in `?mode=client` while operator mode renders byte-stable. Meridian live verification on canonical alias (commit `1dee900`, deployment `dpl_FCCcGrTqfSRCQJ4t9nhyGQHMcqox`): all 20 forbidden strings clean across BOTH report + proposal candidate routes, zero raw UUIDs in subtree, zero Group-B slot id leaks, Group-A still renders (57 SVGs in report subtree), Group-B exhibits still gated, defensive legal/non-binding language preserved, operator mode unchanged. Group-B chip in the report omitted-exhibits appendix now reads "Visuals not included" instead of the prior "GROUP B BLOCK" via a one-case addition to `friendlyExhibitTitle`. Boundary held verbatim: no DB writes, no re-synthesis, no snapshot mutation, no /r mint, no /p mint, no SOW share link, no Send to Client, no email/CRM/Attio/e-sign, no public SOW route, no Group-B unlock, no new public routes, no migrations, no new package dependencies, no Sapient mutation, no real-client mutation, no docs/39 sequence change. **Readiness: client deliverable presentation PASS, client-mode content sanitation PASS, minimum real-pilot readiness PASS.** Full evidence: `docs/61` § 16.

> **Presentation Pass · Pass 2-Fix follow-on note (2026-06-18):** Pass 2 (`26930c8`) crashed in production with `TypeError: (0 , O.g) is not a function` (digest `2113504269`) on both candidate routes in `?mode=client` because `viewerModeFromSearchParam` was exported from a `"use client"` module and consumed by server pages — Next.js converts all exports of a `"use client"` module into client references across the server boundary. Pass 2-Fix (`7660807`) split the helper into `components/reports/viewer-mode.ts` (no `"use client"`); `viewer-mode-toggle.tsx` retains only the client component. Both candidate page imports updated. Lint + build + disclaimer check clean. Live verification confirmed the crash is gone and Group-A exhibits render. Subsequent operator-vocabulary leak in persisted snapshot prose is closed by Pass 3 above.

> **Presentation Pass · Pass 2 follow-on note (2026-06-12):** Items 7.A + 7.B + 7.F from `docs/61` § 7 shipped. The report PDF candidate now renders the three required Group-A visuals (opportunity portfolio 2×2, risk-adjusted priority quadrant, 30/60/90 roadmap Gantt) plus capability maturity heatmap + stakeholder coverage matrix when the engagement data supports them. A new `?mode=client` URL toggle flips both report + proposal candidate documents into a client-facing render that strips UUIDs, claim-guard scan metadata, generated-by labels, snapshot ids, and softens banners + footers to "Discovery report · Draft for review" / "Proposal · Draft for review". Technical "Live chart SVGs are deliberately omitted..." copy replaced with mode-aware language; no adapter codes leak into client copy. Operator drives the Pass 2-aware Meridian UI walkthrough as the post-deploy validation step. Group-B remains gated. Boundary held verbatim. Full evidence: `docs/61` § 15.
